...
namespace Wms12m
{
    ...
    public class Functions
    {
        ...
        public bool isEmail(string email)
        {
            try
            {
                System.Net.Mail.MailAddress a = new System.Net.Mail.MailAddress(email);
            }
            catch { return false; }
        ...
        public bool isNumeric(object value)
        {
            if (value != null)
            {
                ...
                bool isNumeric = reNum.Match(value.ToString()).Success;
                return isNumeric;
            }
            else
            {
        ...
        {
            bool sonuc = false;
            string passComplexity = @"^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{5,100}$";
            Regex re = new Regex(passComplexity);
        ...
        {
            HttpContext context = HttpContext.Current;
            string OS = context.Request.Browser.Platform;
            if (OS == "Unknown")
            {
