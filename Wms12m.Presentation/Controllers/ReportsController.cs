...
namespace Wms12m.Presentation.Controllers
{
    public class ReportsController : RootController
    {
        ...
        {
            if (CheckPerm("Raporlar", PermTypes.Reading) == false) return Redirect("/");
            var KOD = db.Database.SqlQuery<RaporGetKod>(string.Format("[FINSAT6{0}].[wms].[DB_GetKod]", "33")).ToList();
            return View(KOD);
        }
        ...
        {
            if (CheckPerm("Raporlar", PermTypes.Reading) == false) return null;
            var SR = db.Database.SqlQuery<RaporStokKodCase>(string.Format("[FINSAT6{0}].[wms].[StokRaporuKodCase] @Tarih = {1}, @BasGrupKod = '{2}',@BitGrupKod = '{3}', @BasTipKod = '{4}',@BitTipKod= '{5}', @BasOzelKod = '{6}', @BitOzelKod = '{7}',@BasKod1 = '{8}', @BitKod1 = '{9}',@BasKod2= '{10}', @BitKod2 = '{11}',@BasKod3= '{12}', @BitKod3 = '{13}'", "33", Tarih, BasGrupKod, BitGrupKod, BasTipKod, BitTipKod, BasOzelKod, BitOzelKod, BasKod1, BitKod1, BasKod2, BitKod2, BasKod3, BitKod3)).ToList();
        ...
        {
            if (CheckPerm("Raporlar", PermTypes.Reading) == false) return null;
            var BSR = db.Database.SqlQuery<RaporBekleyenSiparis>(string.Format("[FINSAT6{0}].[wms].[BekleyenSiparisRaporu] @BasTarih = {1}, @BitTarih = {2},@BasTeslimTarih = {3}, @BitTeslimTarih = {4}", "33", bastarih, bittarih, basteslimtarih, bitteslimtarih)).ToList();
        ...
        {
            if (CheckPerm("Raporlar", PermTypes.Reading) == false) return null;
            var SU = db.Database.SqlQuery<RaporSatilmayanUrunler>(string.Format("[FINSAT6{0}].[wms].[SatilmayanUrunler] @Number = {1}", "33", gunsayisi)).ToList();
        ...
        {
            if (CheckPerm("Raporlar", PermTypes.Reading) == false) return null;
            var GS = db.Database.SqlQuery<RaporGunlukSatis>(string.Format("[FINSAT6{0}].[wms].[GunlukSatisRaporu] @BasTarih = {1}, @BitTarih = {2}", "33", bastarih, bittarih)).ToList();
        ...
        {
            if (CheckPerm("Raporlar", PermTypes.Reading) == false) return null;
            var CE = db.Database.SqlQuery<RaporCariEkstre>(string.Format("[FINSAT6{0}].[wms].[DB_CariEkstre] @HesapKodu = '{1}'", "33", chk)).ToList();
        ...
        {
            if (CheckPerm("Raporlar", PermTypes.Reading) == false) return null;
            var VGC = db.Database.SqlQuery<RaporVadesiGelmemisCekler>(string.Format("[FINSAT6{0}].[wms].[VadesiGelmemisCekler] @BasTarih = {1}, @BitTarih = {2}", "33", bastarih, bittarih)).ToList();
        ...
        {
            if (CheckPerm("Raporlar", PermTypes.Reading) == false) return Redirect("/");
            var PZ = db.Database.SqlQuery<RaporPozisyonCekSenet>(string.Format("[FINSAT6{0}].[wms].[PozisyonCekSenet]", "33")).ToList();
            return View(PZ);
        }
        ...
        {
            if (CheckPerm("Raporlar", PermTypes.Reading) == false) return null;
            var CLR = db.Database.SqlQuery<RaporCekListesi>(string.Format("[FINSAT6{0}].[wms].[CekListesiRaporu] @Ay = {1}, @IslemTip = {2}, @Yil = {3}", "33", pozisyon, ay, yil)).ToList();
        ...
        {
            if (CheckPerm("Raporlar", PermTypes.Reading) == false) return null;
            var TRB = db.Database.SqlQuery<RaporToplamRiskBakiyesi>(string.Format("[FINSAT6{0}].[wms].[ToplamRiskBakiyesi] @BasTarih = {1}, @BitTarih = {2},@VadeBaslangic = {3}, @VadeBitis = {4},@BasHesapKodu= '{5}', @BitHesapKodu = '{6}'", "33", bastarih, bittarih, basvadetarih, bitvadetarih, chk_bas, chk_bit)).ToList();
        ...
        {
            if (CheckPerm("Raporlar", PermTypes.Reading) == false) return null;
            var GS = db.Database.SqlQuery<RaporCekPortfoyListesi>(string.Format("[FINSAT6{0}].[wms].[CekPortfoyListesi] @BasTarih = {1}, @BitTarih = {2},@BasVadeTarih = {3}, @BitVadeTarih = {4}", "33", bastarih, bittarih, basvadetarih, bitvadetarih)).ToList();
        ...
        {
            if (CheckPerm("Raporlar", PermTypes.Reading) == false) return null;
            var OYM = db.Database.SqlQuery<RaporOdemeYapmayanMusteriler>(string.Format("[FINSAT6{0}].[wms].[OdemeYapmayanMusteriler] @Number = {1}", "33", gunsayisi)).ToList();
        ...
        {
            if (CheckPerm("Raporlar", PermTypes.Reading) == false) return null;
            var BR = db.Database.SqlQuery<RaporBakiye>(string.Format("[FINSAT6{0}].[wms].[BakiyeRaporu] @BasTarih = {1}, @BitTarih = {2},@VadeBaslangic = {3}, @VadeBitis = {4},@BasHesapKodu = '{5}', @BitHesapKodu = '{6}', @Bakiye = {7}", "33", bastarih, bittarih, basvadetarih, bitvadetarih, chk_bas, chk_bit, bakiye)).ToList();
