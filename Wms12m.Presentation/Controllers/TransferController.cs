...
namespace Wms12m.Presentation.Controllers
{
    public class TransferController : RootController
    {
        ...
        public PartialViewResult Stock(string Id)
        {
            ...
            JObject parameters = JsonConvert.DeserializeObject<JObject>(Request["Id"]);
            ...
            string sql = string.Format("SELECT STK.MalKodu, STK.MalAdi, STK.Birim1 as Birim, " +
                                        ...
                                        "WHERE (ISNULL(DST.DvrMiktar, 0) + ISNULL(DST.GirMiktar, 0) - ISNULL(DST.CikMiktar, 0) - ISNULL(DST.KritikStok, 0)) < 0  " +
                                        "ORDER BY DST.MalKodu asc", SirketID, GirisDepo, CikisDepo);
            try
            {
