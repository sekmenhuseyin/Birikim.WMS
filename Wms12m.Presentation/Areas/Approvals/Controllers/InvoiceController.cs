...
namespace Wms12m.Presentation.Areas.Approvals.Controllers
{
    public class InvoiceController : RootController
    {
        ...
        public PartialViewResult List(string Refresh, string ListType)
        {
            ...
            if (Refresh != "")
            {
                FO = db.Database.SqlQuery<FaturaOnay>(string.Format("[FINSAT6{0}].[wms].[FaturaOnayy] @onayTip='{1}'", "17", ListType)).ToList();
        ...
        public ActionResult Detail(string EvrakNo)
        {
            ...
            var FDD = db.MultipleResults(string.Format("[FINSAT6{0}].[wms].[FaturaOnayDetayy] @EvrakNo='{1}'", 17, EvrakNo)).With<FaturaDetayGenel>().With<FaturaDetaySTI>().With<FaturaDetayFTD>().Execute();
        ...
        public string Onay(string EvrakNo, string CHK, string Tarih, short[] ChckSm)
        {
            ...
            try
            {
                var x = db.Database.SqlQuery<int>(string.Format("[FINSAT6{0}].[wms].[FaturaOnayUpdatee] @Tip={1}, @EvrakNo='{2}',@Chk='{3}',@Tarih={4},@RedNedeni='{5}',@Degistiren='{6}',@Degistarih={7}", "17", 1, EvrakNo, CHK, Convert.ToInt32(Convert.ToDateTime(Tarih.ToString()).ToOADate()), "", vUser.UserName, Convert.ToInt32(DateTime.Today.ToOADate()))).ToList();
                return "YES";
            }
            catch (Exception ex)
            {
        ...
        public string Red(string EvrakNo, string CHK, string Tarih, string RedNeden, short[] ChckSm)
        {
            ...
            try
            {
                var x = db.Database.SqlQuery<int>(string.Format("[FINSAT6{0}].[wms].[FaturaOnayUpdatee] @Tip={1}, @EvrakNo='{2}',@Chk='{3}',@Tarih={4},@RedNedeni='{5}',@Degistiren='{6}',@Degistarih={7}", "17", 0, EvrakNo, CHK, Convert.ToInt32(Convert.ToDateTime(Tarih.ToString()).ToOADate()), RedNeden, vUser.UserName, Convert.ToInt32(DateTime.Today.ToOADate()))).ToList();
                return "YES";
            }
            catch (Exception ex)
            {
