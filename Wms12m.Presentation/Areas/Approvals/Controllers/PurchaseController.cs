...
namespace Wms12m.Presentation.Areas.Approvals.Controllers
{
    ...
    public class PurchaseController : RootController
    {
        ...
        public ActionResult GM_Onay()
        {
            ...
            MyGlobalVariables.SipTalepList = db.Database.SqlQuery<SatTalep>(string.Format("[FINSAT6{0}].[wms].[SatinAlmaGMOnayList]", "99")).ToList();
        ...
        public string SipGMOnayListData(string HesapKodu, int SipTalepNo)
        {
            ...
            MyGlobalVariables.TalepSource = db.Database.SqlQuery<SatTalep>(string.Format("[FINSAT6{0}].[wms].[SatinAlmaGMOnayListData] @HesapKodu='{1}', @SipTalepNo={2} ", "99", HesapKodu, SipTalepNo)).ToList();
        ...
        public JsonResult SipGMOnayla()
        {
            ...
            using (KKP kkp = new KKP(ConfigurationManager.ConnectionStrings["WMSConnection"].ConnectionString, "99"))
            {
                try
                {
    ...
    WHERE ID=@ID AND Durum=11 AND SipTalepNo IS NOT NULL", "99");
...
WHERE ID={4} AND Durum=11 AND SipTalepNo IS NOT NULL";
                    ...
                    db.Database.ExecuteSqlCommand(string.Format(sql, vUser.UserName.ToString(), DateTime.Now.ToString("yyyy-dd-MM"), redAciklama, "99", item.ID));
...
GROUP BY (CASE WHEN ST.Birim = STK.Birim1 THEN 1
      ...
      , talepNo, hesapKodu);
        ...
        public PartialViewResult TeklifList()
        {
            var id = Url.RequestContext.RouteData.Values["id"];
            ViewBag.id = id;
            List<SatTeklif> TL = db.Database.SqlQuery<SatTeklif>(string.Format("[FINSAT6{0}].[wms].[TeklifListesi] @MalKodu='{1}'", "99", id)).ToList();
        ...
        public PartialViewResult OnayliTedarikciList()
        {
            var id = Url.RequestContext.RouteData.Values["id"];
            ViewBag.id = id;
            List<SatOnayliTed> OTL = db.Database.SqlQuery<SatOnayliTed>(string.Format("[FINSAT6{0}].[wms].[OnayliTedarikciListesi] @MalKodu='{1}'", "99", id)).ToList();
        ...
        public PartialViewResult SonAlimListesi()
        {
            var id = Url.RequestContext.RouteData.Values["id"];
            ViewBag.id = id;
            List<SonAlimListesi> SAL = db.Database.SqlQuery<SonAlimListesi>(string.Format("[FINSAT6{0}].[wms].[SonAlimListesi] @MalKodu='{1}'", "99", id)).ToList();
