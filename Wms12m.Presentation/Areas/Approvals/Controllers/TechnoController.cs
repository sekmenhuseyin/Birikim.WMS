...
namespace Wms12m.Presentation.Areas.Approvals.Controllers
{
    public class TechnoController : RootController
    {
        ...
        {
            if (CheckPerm(Perms.TechnoIKOnaylama, PermTypes.Reading) == false) return null;
            var list = db.Database.SqlQuery<TechnoList>(string.Format("[HR0312M].[wms].[TCH_EskiUcretSelect] {0}", PERSONELID)).ToList();
        ...
        public string UcretListData(string tip)
        {
            ...
            try
            {
                ucretBilgi = db.Database.SqlQuery<TechnoList>(string.Format("[HR0312M].[wms].[TCH_UcretOnaySelect] @Birim='{1}', @Tip='{0}'", tip, MyGlobalVariables.Birim)).ToList();
            }
            catch (Exception ex)
            {
        ...
        public string PrimListData(string tip)
        {
            ...
            try
            {
                primBilgi = db.Database.SqlQuery<TechnoList>(string.Format("[HR0312M].[wms].[TCH_PrimOnaySelect] @Birim='{1}', @Tip='{0}'", tip, MyGlobalVariables.Birim)).ToList();
            }
            catch (Exception ex)
            {
            ...
            using (var dbContextTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    foreach (JObject insertObj in parameters)
                    {
                        ...
                        string s = string.Format("[HR0312M].[wms].[TCH_UcretOnayUpdate] @OnayDerece={0}, @ID={1},@Birim='{2}'", OnayDerece, ID, MyGlobalVariables.Birim);
            ...
            using (var dbContextTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    foreach (JObject insertObj in parameters)
                    {
                        ...
                        string s = string.Format("[HR0312M].[wms].[TCH_PrimOnayUpdate] @OnayDerece={0}, @ID={1},@Birim='{2}'", OnayDerece, ID, MyGlobalVariables.Birim);
                        var x = db.Database.SqlQuery<int>(s).ToList();
            ...
            using (var dbContextTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    foreach (JObject insertObj in parameters)
                    {
                        ...
                        string s = string.Format("[HR0312M].[wms].[TCH_UcretRedUpdate] @ID={0},@RedNeden='{1}',@Reddeden='{2}'", ID, RedNeden, vUser.UserName.ToString());
                        var x = db.Database.SqlQuery<int>(s).ToList();
            ...
            using (var dbContextTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    foreach (JObject insertObj in parameters)
                    {
                        ...
                        string s = string.Format("[HR0312M].[wms].[TCH_PrimRedUpdate] @ID={0},@RedNeden='{1}',@Reddeden='{2}'", ID, RedNeden, vUser.UserName.ToString());
                        var x = db.Database.SqlQuery<int>(s).ToList();
