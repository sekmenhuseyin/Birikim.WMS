namespace Wms12m.Entity
{
    public class WebSiparis
    {
        ...
        public static string Sorgu = @"
									...
									case (select count(*) FROM FINSAT6{0}.FINSAT6{0}.SPI (NOLOCK) SS  WHERE SS.EVRAKNO=SPI.EvrakNo AND SS.Tarih=SPI.Tarih and SPI.KynkEvrakTip=SS.KynkEvrakTip and SS.CHK=SPI.CHK and ss.SiparisDurumu=0) when 0 then 'Kapalı' else 'Açık' end as SiparisDurumu 
                                    ...
                                    {3}
                                    GROUP BY   SPI.Tarih,SPI.EvrakNo,CHK.TipKod,CHK.GrupKod,SPI.Chk,CHK.Unvan1,CHK.Unvan2,SPI.KynkEvrakTip
									) AS D
									ORDER BY D.EVRAKNO
                                    ";
    }
